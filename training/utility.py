...
def train_network(model: tf.keras.models.Model, model_name: str, x_train, y_train=None):
    ...
    if y_train is None:
        ...
        model.fit(loader.load(), steps_per_epoch=100, epochs=100, batch_size=232, use_multiprocessing=True, verbose=1)
    else:
        model.fit(x=x_train, y=y_train, epochs=100, batch_size=232, use_multiprocessing=True, verbose=1)
