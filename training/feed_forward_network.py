...
def train_feed_forward_network(n_output_labels, x_train, y_train):
    ffn = FeedForwardNetwork(n_label=n_output_labels, activation='relu', output_activation='linear', use_bias=True)
    return utility.train_network(ffn, "feed_forward_network", x_train, y_train)
